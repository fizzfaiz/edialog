        ...
        <div class="card">
            <div class="card-header">
                <div class="float-start">
                    <i class="bi bi-person-gear me-2"></i>Edit Pengguna: <strong>{{ $user->name }}</strong>
                </div>
                <div class="float-end">
            ...
            <div class="card-body">
                <form action="{{ route('users.update', $user->id) }}" method="post">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-6">
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $user->name) }}" placeholder="Nama penuh">
                            @error('name')
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-6">
                            <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $user->email) }}" placeholder="nama@example.com">
                            @error('email')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            <select class="form-select @error('pejabat_pendidikan_id') is-invalid @enderror" id="pejabat_pendidikan_id" name="pejabat_pendidikan_id" onchange="loadSektors(this.value)">
                                ...
                                @foreach ($pejabats as $pejabat)
                                    <option value="{{ $pejabat->id }}" {{ old('pejabat_pendidikan_id', $user->pejabat_pendidikan_id) == $pejabat->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3 row">
                        <div class="col-md-6 offset-md-4">
                            <button type="submit" class="btn btn-primary btn-lg">
                                <i class="bi bi-save"></i> Kemaskini Pengguna
                            </button>
                        </div>
                    </div>
    ...
    function loadSektors(pejabatId) {
        ...
        fetch('/api/sektors/' + pejabatId)
            ...
            .then(data => {
                ...
                const selectedSektor = '{{ old('sektor_id', $user->sektor_id ?? '') }}';
                if (selectedSektor) {
                    sektorSelect.value = selectedSektor;
                    loadUnits(selectedSektor);
                }
            })
            .catch(err => {
    ...
    function loadUnits(sektorId) {
        ...
        fetch('/api/units/' + sektorId)
            ...
            .then(data => {
                ...
                const selectedUnit = '{{ old('unit_id', $user->unit_id ?? '') }}';
                if (selectedUnit) {
